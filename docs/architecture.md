...
- **Backend (`apps/api/`)**
  ...
  - Runtime credential rows now persist non-secret connector settings in `settings` plus encrypted secret payloads in `secret_material_encrypted`, replacing the old plaintext `config` blob while keeping existing `connection_id` bindings stable.
