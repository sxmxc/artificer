...
- **Backend (`apps/api/`)**
  ...
  - Runtime credential rows now persist non-secret connector settings in `settings` plus encrypted secret payloads in `secret_material_encrypted`, replacing the old plaintext `config` blob while keeping existing `connection_id` bindings stable. Postgres DSN-mode secrets may still arrive or exist under `dsn`, `database_url`, or `url`, redacted admin updates preserve those values across that alias normalization, and redacted HTTP header placeholders now preserve existing secrets with case-insensitive header-name matching.
